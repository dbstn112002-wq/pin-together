...
function openChecklistDetail(id, returnPanel='checklists', returnPinId='') {
  ...
  $('#checklistDetailItems').innerHTML = list.checklist_items.map((item, index) => `<div class="checklist-item ${item.is_checked ? 'checked' : ''}"><button type="button" class="checklist-item-toggle" data-checklist-item="${item.id}" data-checklist-id="${id}"><span>${item.is_checked ? '☑' : '☐'}</span>${escapeHtml(item.label)}</button>${editable ? `<div class="checklist-item-actions"><button type="button" data-edit-checklist-item="${item.id}" title="수정">✎</button><button type="button" data-move-checklist-item="${item.id}" data-direction="-1" ${index === 0 ? 'disabled' : ''} title="위로">↑</button><button type="button" data-move-checklist-item="${item.id}" data-direction="1" ${index === list.checklist_items.length - 1 ? 'disabled' : ''} title="아래로">↓</button><button type="button" data-delete-checklist-item="${item.id}" title="삭제">×</button></div>` : ''}</div>`).join('');
...
async function moveChecklistItem(checklistId, itemId, direction) { const list = state.checklists.find(item => item.id === checklistId), index = list?.checklist_items.findIndex(item => item.id === itemId), target = list?.checklist_items[index + direction]; if (!list || index < 0 || !target) return; const item = list.checklist_items[index]; const { error } = await sb.from('checklist_items').upsert([{ id:item.id, checklist_id:checklistId, label:item.label, position:target.position, is_checked:item.is_checked }, { id:target.id, checklist_id:checklistId, label:target.label, position:item.position, is_checked:target.is_checked }]); if (error) return toast(error.message); await loadChecklists(); reopenChecklistDetail(checklistId); }
