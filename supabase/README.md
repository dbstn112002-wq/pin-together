...
14. `scheduled-pins-migration.sql`
15. `active-announcement-migration.sql`
16. `announcement-preferences-migration.sql`
...
`polls-migration.sql`은 투표, 항목, 투표 기록, 핀 연결(`poll_pin_links`), 생성자 전용 삭제 정책과 새 투표 알림을 만듭니다. 투표 알림을 켠 현재 공간 참여자에게만 생성 알림을 보냅니다. 이미 실행한 경우에도 최신 정책과 핀 연결 테이블을 반영하기 위해 같은 파일을 다시 실행해도 안전합니다.

`checklists-migration.sql`은 개인·공간·핀 체크리스트와 항목, 실시간 동기화, 체크리스트 활동 알림을 만듭니다. 체크리스트 알림을 켠 공간 참여자에게 공간·핀 목록과 항목의 생성·수정·삭제·체크/해제 알림을 보냅니다. 체크리스트 기능을 사용하기 전에 반드시 실행합니다.
